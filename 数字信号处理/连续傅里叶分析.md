...
$$
A x(t) + B y(t) \;\longleftrightarrow\; A a_k + B b_k
$$

**时移**：

$$
x(t-t_0) \;\longleftrightarrow\; a_k\, e^{-jk\omega_0 t_0}
$$
...
$$
e^{jl\omega_0 t} x(t) \;\longleftrightarrow\; a_{k-l}
$$

频移对应 FS 系数在离散频率轴上的平移（下标偏移）。
...
$$
x(at) \;\longleftrightarrow\; a_k \quad (\text{基频变为 } |a|\omega_0)
$$

尺度变换改变基频，但 FS 系数 $a_k$ 本身的形式不变（对应的积分区间尺度随之缩放）。
...
$$
\frac{d^n x(t)}{dt^n} \;\longleftrightarrow\; (jk\omega_0)^n\, a_k
$$

**一阶积分**：

$$
\int x(t)dt \;\longleftrightarrow\; \frac{a_k}{jk\omega_0} \quad (k \neq 0)
$$

> **注**：时域乘以 $t$ 会破坏周期性，因此 FS 不存在频域微分/积分性质。
...
信号的**周期平均功率**等于各谐波功率之和，表明 FS 是保功率的正交分解。
...
> **统一观点**：FS 和 FT 是同一数学框架在不同信号类型下的具体形式，核心都是**信号在复指数基上的正交分解**。FS 作为 FT 的特例：周期信号的 FT 为 $2\pi\sum a_k\delta(\omega-k\omega_0)$；FT 是 FS 在 $T\to\infty$ 时的极限推广。
...
周期信号也可纳入 FT 框架。周期信号 $x(t) = \sum_{k=-\infty}^{+\infty} a_k e^{jk\omega_0 t}$ 的傅里叶变换为：
...
周期信号的 FT 是**离散冲激序列**，冲激位于谐波频率 $k\omega_0$，强度为 $2\pi a_k$。FS 系数 $a_k$ 与 FT 之间的关系为：$a_k$ 是 FT 在离散频率点的"加权采样"。
...
$$
A x(t) + B y(t) \;\longleftrightarrow\; A X(j\omega) + B Y(j\omega)
$$
...
$$
\frac{d^n x(t)}{dt^n} \;\longleftrightarrow\; (j\omega)^n X(j\omega)
$$
...
$$
t\,x(t) \;\longleftrightarrow\; j\frac{dX(j\omega)}{d\omega}, \qquad
t^n x(t) \;\longleftrightarrow\; j^{\,n}\frac{d^{\,n}X(j\omega)}{d\omega^{\,n}}
$$
...
若 $x(t)$ 为实信号，则 $X(-j\omega) = X^*(j\omega)$。由此：
