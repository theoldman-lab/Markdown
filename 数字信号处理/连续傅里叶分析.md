...
时域乘以 $t$ 对应频域微分。由傅里叶变换定义对 $\omega$ 求导可得：
$$
\begin{aligned}
\text{一阶：}\quad & t\,x(t) \;\longleftrightarrow\; j\frac{dX(j\omega)}{d\omega} \\
\text{$n$ 阶：}\quad & t^n x(t) \;\longleftrightarrow\; j^{\,n}\frac{d^{\,n}X(j\omega)}{d\omega^{\,n}}
\end{aligned}
$$
...
$$
\frac{x(t)}{t} \;\longleftrightarrow\; -j\int_{-\infty}^{\omega} X(j\Omega)\,d\Omega \;+\; \pi x(0)\,\delta(\omega)
$$

其中第二项仅在 $x(0)\neq0$ 时出现。若 $x(0)=0$，则简化为 $\displaystyle -j\int_{-\infty}^{\omega} X(j\Omega)\,d\Omega$。
...
>
> **解**：利用频域微分性质 $t x(t) \leftrightarrow j\frac{dX(j\omega)}{d\omega}$：
>
> $$
> \begin{aligned}
